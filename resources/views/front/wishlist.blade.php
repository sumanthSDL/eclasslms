                                    ...
                                    <div class="view-block">
                                        <div class="view-img">
                                            @if ($wish->courses['preview_image'] !== null && $wish->courses['preview_image'] !== '')
                                                <a
                                                    href="{{ route('user.course.show', ['id' => $wish->courses->id, 'slug' => $wish->courses->slug]) }}"><img
                                                        src="{{ asset('images/course/' . $wish->courses->preview_image) }}"
                                                        class="img-fluid" alt="course">
                                                @else
                                                    <a
                                                        href="{{ route('user.course.show', ['id' => $wish->courses->id, 'slug' => $wish->courses->slug]) }}"><img
                                                            src="{{ Avatar::create($wish->courses->title)->toBase64() }}"
                                                            class="img-fluid" alt="course">
                                            @endif
                                            </a>
                                        </div>
                                        <div class="view-user-img">
                                            @if ($wish->user['user_img'] !== null && $wish->user['user_img'] !== '')
                                                <a href="{{ route('all/profile', $wish->user->id) }}" title=""><img
                                                        src="{{ asset('images/user_img/' . $wish->user['user_img']) }}"
                                        ...
                                        <div class="view-dtl">
                                            <div class="view-heading"><a
                                                    href="{{ route('user.course.show', ['id' => $wish->courses->id, 'slug' => $wish->courses->slug]) }}">{{ str_limit($wish->courses->title, $limit = 35, $end = '...') }}</a>
                                            </div>
                                            <div class="user-name">
                                                <h6>By <span><a href="{{ route('all/profile', $wish->user->id) }}">
                                                            {{ optional($wish->courses->user)['fname'] }}</a></span></h6>
                                            </div>
                                            <div class="rating">
                                                <ul>
                                                    <li>
                                                        ...
                                                        $sub_total = 0;
                                                        $sub_total = 0;
                                                        $reviews = App\ReviewRating::where('course_id', $wish->courses->id)
                                                            ->where('status', '1')
                                                            ->get();
                                                        ?>
                                                        @if (!empty($reviews[0]))
                                                    ...
                                                    $reviewcount = App\ReviewRating::where('course_id', $wish->courses->id)
                                                        ->where('status', '1')
                                                        ->WhereNotNull('review')
                                                        ->get();
                                            ...
                                            <ul>
                                                    ...
                                                    @if ($wish->courses->type == 1)
                                                        <div class="cart-btn">
                                                            <form id="demo-form2" method="post"
                                                                action="{{ route('addtocart', ['course_id' => $wish->courses->id, 'price' => $wish->courses->price, 'discount_price' => $wish->courses->discount_price]) }}"
                                                                data-parsley-validate
                                                                class="form-horizontal form-label-left">
                                                                {{ csrf_field() }}

                                                                <input type="hidden" name="category_id"
                                                                    value="{{ $wish->courses->category['id'] }}" />


                                                                <button type="submit" class="btn btn-primary wishlist-cart"
                                                                    title="Add To Cart"><i data-feather="shopping-cart"
                                                ...
                                                <li class="protip-wish-btn-two">
                                                    <div class="wishlist-btn txt-rgt">
                                                        <form method="post"
                                                            action="{{ url('delete/wishlist', $wish->id) }}"
                                                            data-parsley-validate class="form-horizontal form-label-left">
                                                            {{ csrf_field() }}

                                                            <button type="submit" class="btn btn-primary trash-icon"
                                                                title="Remove From Wishlist"><i data-feather="trash"
                                                                    class="rgt-10"></i></button>
                                                        </form>
                                                    </div>
                                                </li>
